...
const iconPaths: Record<IconName, React.ReactElement> = {
  ...
  // New structural engineering icons
  building: (
...
const Icon: React.FC<IconProps> = ({ name, size = 'sm', className = '' }) => {
  const sizeClass = sizeClasses[size];
  const isLoading = name === 'loading';
  
  return (
    <svg 
      className={`${sizeClass} ${isLoading ? 'animate-spin' : ''} ${className}`}
      fill={name === 'error' || name === 'warning' ? 'currentColor' : 'none'}
      stroke={name === 'error' || name === 'warning' ? 'none' : 'currentColor'}
      viewBox={name === 'error' || name === 'warning' ? '0 0 20 20' : '0 0 24 24'}
    >
      {iconPaths[name]}
    </svg>
  );
